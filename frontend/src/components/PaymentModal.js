...
import { addPayment } from '../redux/actions/Payments'; // Acción que crearás para procesar el pago

function PaymentModal({ member, closePaymentModal }) {
  const dispatch = useDispatch();
  
  // Obtener actividades del store en caso de que solo tengas IDs de actividades
  ...
  const handlePayment = () => {
    const paymentData = {
      ...
      activities: member.activities, // Asumiendo que ya contiene los IDs de actividades
    };

    // Despachar la acción para registrar el pago
    dispatch(addPayment(paymentData));
    closePaymentModal(); // Cerrar el modal después de confirmar el pago
  };

  // Buscar los nombres de las actividades en caso de que solo tengamos los IDs
